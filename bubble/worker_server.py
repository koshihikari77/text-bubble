...
def _load_mask(path: str | None):
    if not path:
        return None
    from bubble.scene_planners.cp_sat import load_binary_mask

    return load_binary_mask(Path(path))


def _load_mask_bundle_from_payload(payload: dict[str, Any]) -> Any:
    person_mask = payload.get("person_mask")
    face_mask = payload.get("face_mask")
    if not person_mask or not face_mask:
        return None
    return _scene_runtime().load_mask_bundle(
        person_mask_path=Path(str(person_mask)),
        face_mask_path=Path(str(face_mask)),
        chest_mask_path=Path(str(payload["chest_mask"])) if payload.get("chest_mask") else None,
        lower_mask_path=Path(str(payload["lower_mask"])) if payload.get("lower_mask") else None,
        head_mask_path=Path(str(payload["head_mask"])) if payload.get("head_mask") else None,
    )
...
def _solve_cp_sat_scene_command(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    masks = _load_mask_bundle_from_payload(payload)
    if masks is None:
        raise RuntimeError("cp-sat scene solving requires person and face masks")
    bundle = _scene_runtime().solve_cp_sat_scene_bundle(
        dialogue_lines=dialogue_lines,
        reflow_plans=reflow_plans,
    ...
    solution = bundle.evaluated_solution
    return {
        "status": "ok",
        "dialogue_lines": dialogue_lines,
...
def _run_pipeline_command(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    result = _scene_runtime().run_pipeline(
        ...
        image_width=image_width,
        image_height=image_height,
        planner=str(payload.get("planner", _scene_runtime().default_scene_planner())),
        masks=_load_mask_bundle_from_payload(payload),
        font_size=int(payload["font_size"]),
    )
