...
def _run_single_payload_in_process(payload: dict[str, str]) -> tuple[str, Path]:
    from bubble.experimental import poc_scene_place_from_masks as poc_module
    ...
    if int(result["exit_code"]) != 0:
        ...
        try:
            ...
            from bubble.scene_planners import cp_sat as cp_sat_scene_solver

            reload(cp_sat_scene_solver)
            result = poc_module.run_args(namespace, emit_paths=False)
        finally:
